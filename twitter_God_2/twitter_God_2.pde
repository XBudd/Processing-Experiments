...
int interval = 5000; // 
int time;
boolean Chaos_It_Value;
boolean chaosOn = false;
// States
final int enterWord = 0;
final int wordEntered = 1;
int state = enterWord;
...
void enterWord() {
  ...
  strokeWeight(2);
  fill(0, 51, 102);
  rect((width/2)-335, (height/2)-20, 670, 80, 10);
  println(tweets.size());
  fill(255);
  cp5 = new ControlP5(this);
  cp5.disableShortcuts();
  PFont font = createFont("arial", 20);
  myTextfield = cp5.addTextfield("Enter Your Term")
    .setPosition((width/2)-320, height/2)
    .setSize(370, 40)
    .setFont(font)
    .setFocus(true)
    .setColor(color(255))
  ...
  cp5.addButton("Start")
    .setPosition((width/2)+60, height/2)
    .setSize(80, 40)
    .setColorBackground(color(50, 200, 100))
  ...
  cp5.addButton("Pause")
    ...
    .setSize(80, 40)
    .setColorBackground(color(40, 104, 250))
    .getCaptionLabel().align(ControlP5.CENTER, ControlP5.CENTER)
    ;

  myToggle = cp5.addToggle("Chaos_It_Value")
    .setPosition((width/2)+240, height/2)
    .setSize(80, 40)
    .setState(false)
    ;
  String b = "";
  if (Chaos_It_Value == false) {
    myToggle
      .setColorBackground(color(255, 0, 50));
    b = "Chaos It";
  } else {
    myToggle
      .setColorBackground(color(255, 200, 40));
    b = "Cool It";
  }
  myToggle
    .setCaptionLabel(b)
  ...
  text(cp5.get(Textfield.class, "Enter Your Term").getText(), 360, 130);
...
public void Start() {
  ...
  w = myTextfield.getText();
  if (w != "" && !w.isEmpty()) {
    println(w);
    tweetQuery(w);
  } else {
    text("Please enter a non-null word", 50, 50);
    w ="";
  }
}
...
public void Chaos_It_Value(boolean theValue) {
  if (!theValue) {
     chaosOn = false;
     frameRate(10);
     interval =5000;
  ...
  if (state == wordEntered) {
    ...
    try {
      if (millis() - time > interval) {
        ...
        /*  cp5.addButton("imgButton")
            .setPosition(random(width/2), random(height/2))
            .setImages(webImg, webImg, webImg)
            .updateSize(); */
           
          //image(webImg, random(width/2), random(height/2));
         button = new ImageButton(webImg, random(width/2), random(height/2), imgList[p]);
         button.display();
         button.update();
         //image(button.currentimage, button.imgW, button.imgH);
...
class ImageButton extends Button{
  ...
  String imgLink;
  String name;
  ImageButton(PImage i, float w, float h, String u) {
    //image(i, w, h);
    //imgLink = u;
    currentimage=i;
    imgW = w;
...
public void imgButton(int theValue){
  println("a button event from buttonA: " +theValue);
  
}
// Class for Tweet Query

void tweetQuery(String w) {
